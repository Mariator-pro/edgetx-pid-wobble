...
The script refuses to run unless it can read the current flight mode. Without the `FM` sensor it stays silent, with no error message.
...
5. Go back to the Mixer overview and repeat steps 2 to 4 for the **Pitch / Ele** channel, this time picking `1-wobble/Pitch` as the source.
...
4. Confirm that the wobble stops immediately as soon as **any** activation condition is no longer met, for example flip the Wobble switch off, the Enable switch off, or switch the FC out of Angle mode. The roll and pitch outputs must jump back to `0` in the same tick, with no fade or held last value.
